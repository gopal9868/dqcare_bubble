...
const gettestcasefull=async (testCaseName,searchType)=>{
    if (searchType=='search') {
        var search_url=`${apiUrl}tcrescompareview/?search=${testCaseName}`  
    }
    else {
    var search_url=`${apiUrl}tcrescompareview/?appname=${testCaseName}`} 
  ...
  //console.log(table_columns);
   var s='<table id="table_1" class="table table-hover table-bordered"><tr>'
    ...
    data.forEach(row => {
      ...
      <td>${row['result_column']}</td><td>${row['pk_column']}</td><td id="srcconame_${row['id']}">${row['pk_value']}</td>
      <td>${row['source_result']}</td><td>${row['target_result']}</td><td>${row['result_desc']}</td>
